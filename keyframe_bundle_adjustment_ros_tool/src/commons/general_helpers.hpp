 ...
 */
        ...
        void toGeometryMsg_mc(geometry_msgs::Pose &out, const Eigen::Transform<double, 3, TransformType> &pose)
        {
 ...
 */
        ...
        void saveTrajectory(std::string filename, std::string tf_parent_frame_id, const keyframe_bundle_adjustment::BundleAdjusterKeyframes::Ptr &bundle_adjuster_)
        {
            std::ofstream path_data(filename.c_str());
            // path_data.precision(12);
            nav_msgs::Path path_msg;
            // timestamp of msg is same as last keyframe
            ros::Time cur_ts;
            cur_ts.fromNSec(bundle_adjuster_->getKeyframe().timestamp_);
            ...
            for (const auto &kf : bundle_adjuster_->keyframes_)
            {
                ...
                toGeometryMsg_mc(cur_pose.pose, kf.second->getEigenPose().inverse());
